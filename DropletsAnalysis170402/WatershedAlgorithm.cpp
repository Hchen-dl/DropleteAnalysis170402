...
WatershedAlgorithm::WatershedAlgorithm(Mat image)
{
	src = image;
}
Mat WatershedAlgorithm::mainProcess()
{
	cvtColor(src, src_gray, CV_RGB2GRAY);
	GaussianBlur(src_gray, src_gray, Size(5, 5), 5); //滤波  
	binary = Mat::zeros(src.size(), CV_8UC3);
...
Mat WatershedAlgorithm::DistanceTransform(Mat binary,Mat dstSrc_Bw)
{
	Mat src_bw(src_gray.size(), CV_32FC1);
	Mat src_BW(src_gray.size(), CV_8UC1);
	distanceTransform(binary, src_bw, CV_DIST_L12, 0);  //距离变换
...
void WatershedAlgorithm::fillHole(Mat srcBw, Mat &dstBw)
{
